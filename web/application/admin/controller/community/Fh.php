 ...
 * @icon fa fa-circle-o
 */
class Fh extends Backend
{

    /**
     * Fh模型对象
     * @var \app\admin\model\Fh
     */
    ...
    {
        parent::_initialize();
        $this->model = new \app\admin\model\Fh;
        $list = ['待审核','已通过','已拒绝'];
                ...
                try {
                    ...
                    if(config('site.auto_comment') && isset(input()['comments'])) {
                        ...
                        for ($i = 0; $i < $size; $i++) {
                            ...
                            Comment::insert(['name' => $name, 'avator_image' => $photo, 'content' => $comment[$i], 'community_id' => $result, 'class' => 0, 'creat_time' => date('Y-d-m H:i:s')]);
                        }
                    }
                    Db::commit();
